...
- **ダブルブラインドレビュー**: 滞在終了後に双方が評価(総合+清潔さ/コミュニケーション/掲載との一致)。双方が投稿するか終了40日後に同時公開され、報復レビューを防止。ホストは公開後1回だけ返信可
- **本人確認バッジ**: メール認証済み/電話番号認証済み(SMS OTP)をプロフィール・掲載ページに表示。※SMS送信はSupabaseにTwilio等のプロバイダ設定が必要
- **通報・ブロック**: 掲載・ユーザーの通報(運営のみ閲覧)。ブロック中の相手とは新規会話・メッセージ・交換リクエストがRLSレベルで不可
